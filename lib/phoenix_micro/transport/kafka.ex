defmodule PhoenixMicro.Transport.Kafka do
  ...
  defp commit_offset(state, group_id, gen_id, member_id, topic, partition, offset) do
    with {:ok, conn} <- Connection.open(state.brokers),
         request <-
           Protocol.offset_commit_request(
             group_id,
             gen_id,
             member_id,
             topic,
...
defmodule PhoenixMicro.Transport.Kafka.Protocol do
  ...
  defp record_batch(key, value) do
    ...
    (<<0::64, 0::32, -1::32-signed, 2::8, 0::32, 0::16, 0::32, ts::64, ts::64, -1::64-signed,
       -1::16-signed, 0::32, 1::32>> <> records)
    |> then(fn batch ->
  ...
  def fetch_request(topic, partition, offset, max_bytes, max_wait_ms, client_id) do
    body =
      <<-1::32-signed, max_wait_ms::32, 1::32, max_bytes::32, 0::8, 1::32>> <>
        enc_str(topic) <>
  ...
  def parse_fetch(<<_cid::32, _throttle::32, 0::16, _session_id::32, count::32, rest::binary>>)
      when count > 0 do
  ...
  defp parse_fetch_parts(bin, n, acc) do
    case bin do
      <<_partition::32, 0::16-signed, _hw::64, _ls::64, _log::64, _aborted::32, blen::32,
        batch::binary-size(blen), rest2::binary>> ->
  ...
  defp decode_batch(
         <<base::64, _blen::32, _epoch::32-signed, 2::8, _crc::32, _attrs::16, _last::32,
           _fts::64, _mts::64, _pid::64-signed, _pepoch::16-signed, _seq::32, rcount::32,
           records::binary>>
       ) do
  ...
  def parse_list_offsets(<<_cid::32, _thr::32, 1::32, rest::binary>>) do
    case rest do
      <<tl::16, _topic_name::binary-size(tl), 1::32, _partition::32, 0::16-signed, _timestamp::64,
        off::64, _rest::binary>> ->
        {:ok, off}
  ...
  def parse_find_coordinator(
        <<_cid::32, _throttle::32, 0::16-signed, err_mlen::16, _err_msg::binary-size(err_mlen),
          _node_id::32, hlen::16, host::binary-size(hlen), port::32, _rest::binary>>
      ) do
    {:ok, {host, port}}
  rescue
    _other -> {:error, :parse_error}
  ...
  def parse_join_group(
        <<_cid::32, _throttle::32, 0::16-signed, gen_id::32-signed, ptl::16,
          _proto_type::binary-size(ptl), pnl::16, _proto_name::binary-size(pnl), ll::16,
          leader::binary-size(ll), ml::16, member_id::binary-size(ml), mc::32, rest::binary>>
      ) do
    members = parse_members(rest, mc, [])
    am_leader = leader == member_id
    # single-partition default
    assigned = if am_leader, do: [0], else: [0]
  ...
  def sync_group_request(group_id, join_resp, client_id) do
    ...
    assignments =
      if is_leader do
        asgn =
          Enum.map(join_resp.members, fn mid ->
            payload =
              <<0::16, 1::32, byte_size(join_resp.leader)::16, join_resp.leader::binary, 1::32,
                0::32, -1::32-signed>>
  ...
  def parse_sync_group(
        <<_cid::32, _throttle::32, 0::16-signed, al::32, asgn::binary-size(al), _rest::binary>>,
        fallback
      ) do
  ...
  defp decode_asgn_topics(
         <<topic_len::16, _topic::binary-size(topic_len), pc::32, rest::binary>>,
         n,
         acc
       ) do
